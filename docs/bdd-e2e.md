...
fake OpenHands client. It does not require GitHub credentials, a network call, a
real OpenHands service, or PostgreSQL.
...
| Given a queued review run, when a worker acquires it, then the run is locked, a local workspace is prepared from a temporary git repo, and an OpenHands session receives the commit-range input. | Same P0 test |
| Given a completed OpenHands JSON result, when the result is collected, then schema validation marks publishable line findings, keeps invalid diff locations as summary-only, persists findings, upserts one summary comment ref, and dedupes line comment refs. | Same P0 test |
...
| OpenHands failed, timeout, cancelled, non-JSON output, missing fields, or unpublishable line locations. | API/component tests cover session failure and parser validation; E2E currently covers one summary-only location. |
...
- `tests/fixtures/openhands_review_result.json`

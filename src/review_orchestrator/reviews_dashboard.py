...
async function loadRuns({background=false,announce=true}={}){if(state.loading&&background)return;state.controller?.abort();const controller=new AbortController();state.controller=controller;state.loading=true;$('#refresh').disabled=true;$('#refresh').classList.add('is-loading');$('#results').setAttribute('aria-busy','true');if(!background)$('#results').innerHTML=renderLoading();try{const response=await fetch(`${API}?${params()}`,{headers:{Accept:'application/json'},signal:controller.signal});if(!response.ok)throw new Error(errorMessage(response));const data=await response.json();state.total=Number(data.total)||0;const items=Array.isArray(data.items)?data.items:[];$('#results').innerHTML=items.length?renderRows(items):renderEmpty();$('#result-summary').textContent=`共 ${state.total} 次执行，本页显示 ${items.length} 条`;if(updatePagination()){setTimeout(()=>loadRuns(),0);return}const now=new Date();$('#updated-at').textContent=`更新于 ${new Intl.DateTimeFormat('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(now)}`;if(announce)$('#announcer').textContent=`Review 列表已更新，共 ${state.total} 条记录`;state.remaining=REFRESH_SECONDS;syncUrl()}catch(error){if(error.name==='AbortError')return;$('#results').innerHTML=renderError(error.message||'无法连接 review 服务，请检查网络后重试。');$('#result-summary').textContent='未能取得任务数据';$('#pagination').hidden=true;$('#announcer').textContent='Review 列表加载失败'}finally{if(state.controller===controller){state.loading=false;$('#refresh').disabled=false;$('#refresh').classList.remove('is-loading');$('#results').setAttribute('aria-busy','false');updateRefreshLabel()}}}

...
function mrCell(run){const context=run.pull_request_context||{};const provider=String(run.provider||'').toLowerCase();const meta=providerMeta[provider]||{};const kind=String(meta.kind||provider);const prefix=kind==='gitlab'?'!':'#';const requestName=kind==='gitlab'?'Merge request':'Pull request';const title=context.title||`${requestName} ${prefix}${run.pull_request_number}`;const url=safeUrl(context.html_url);const titleHtml=url?`<a class="mr-title" href="${esc(url)}" target="_blank" rel="noopener noreferrer" title="${esc(title)}">${esc(title)}</a>`:`<span class="mr-title" title="${esc(title)}">${esc(title)}</span>`;const mark=String(meta.display_name||provider||'?').split(/[\s_-]+/).map(part=>part[0]||'').join('').slice(0,3).toUpperCase();return `<div class="mr"><span class="provider-mark ${kind==='gitlab'?'gitlab':''}" aria-hidden="true">${esc(mark)}</span><span>${titleHtml}<span class="mr-number">${prefix}${esc(run.pull_request_number)} · 第 ${esc(run.attempt)} 次执行</span></span></div>`}

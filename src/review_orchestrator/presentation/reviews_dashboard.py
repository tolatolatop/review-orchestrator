# ruff: noqa: E501
"""Bundled, dependency-free review execution dashboard."""
...
.rerun-button,.retry-button{height:34px;white-space:nowrap;border:1px solid #c6a469;border-radius:7px;background:#fffaf0;color:#7b4c08;padding:0 11px;cursor:pointer;font-weight:700}
.rerun-button:hover,.retry-button:hover{border-color:var(--amber);background:var(--amber-soft)}
.rerun-button[disabled],.retry-button[disabled]{cursor:wait;opacity:.6}
.action-muted{color:#9aa4b4;font-size:11px}
...
tbody tr.highlight{animation:highlight-run 2.4s ease-out}
...
.rerun-dialog{width:min(520px,calc(100% - 28px));border:0;border-radius:13px;padding:0;background:var(--panel);color:var(--ink);box-shadow:0 30px 90px rgba(14,25,43,.28)}
.rerun-dialog::backdrop{background:rgba(15,27,45,.58);backdrop-filter:blur(2px)}
.dialog-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:20px 22px 17px;border-bottom:1px solid var(--line)}
.dialog-head h2{margin:0;font:750 21px/1.2 ui-rounded,"Arial Rounded MT Bold","PingFang SC",sans-serif;letter-spacing:-.025em}
.dialog-head p{margin:5px 0 0;color:var(--muted);font-size:12px}
.dialog-close{width:32px;height:32px;border:1px solid var(--line);border-radius:7px;background:#fff;color:var(--muted);cursor:pointer;font-size:18px}
.dialog-body{padding:20px 22px}
.revision-seal{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;border:1px solid #d9c18f;border-radius:10px;background:#fffaf0;padding:14px 16px;color:#6d470c}
.revision-seal span{display:grid;gap:3px}
.revision-seal span:last-child{text-align:right}
.revision-seal small{color:#9a7740;font:700 9px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.1em;text-transform:uppercase}
.revision-seal code{font:750 13px ui-monospace,SFMono-Regular,Menlo,monospace}
.revision-seal b{font-size:17px;color:var(--amber)}
.attempt-shift{margin:13px 0 0;color:var(--ink);font-weight:700}
.dialog-note{margin:8px 0 0;color:var(--muted);font-size:12px}
.dialog-error{margin:12px 0 0;border-left:3px solid var(--red);background:var(--red-soft);padding:9px 11px;color:var(--red);font-size:12px}
.dialog-actions{display:flex;justify-content:flex-end;gap:9px;padding:16px 22px;border-top:1px solid var(--line);background:#fbfcfe}
.dialog-actions button{height:38px;border-radius:7px;padding:0 15px;cursor:pointer;font-weight:700}
.dialog-actions button[disabled]{cursor:wait;opacity:.65}
.toast{position:fixed;right:22px;bottom:22px;z-index:20;display:flex;align-items:center;gap:14px;max-width:min(430px,calc(100% - 28px));border:1px solid #b9ddcf;border-radius:10px;background:#f2fbf7;padding:12px 14px;color:#155f47;box-shadow:0 16px 45px rgba(23,32,51,.16)}
.toast[hidden]{display:none}
.toast strong{display:block}
.toast span{font-size:12px;color:#3d7564}
.toast button{white-space:nowrap;border:0;background:transparent;color:var(--blue);cursor:pointer;font-weight:750}
.visually-hidden{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}
@keyframes spin{to{transform:rotate(360deg)}}
@keyframes shimmer{to{background-position:-100% 0}}
@keyframes highlight-run{0%{background:var(--amber-soft)}100%{background:transparent}}
...
@media(max-width:420px){h1{font-size:25px}.identity{gap:11px}.mark{width:40px;height:40px}.refresh-box{align-items:flex-end}.icon-button span{display:none}.rail{width:100%;max-width:248px}.list-meta{align-items:flex-start;flex-direction:column}.pagination{flex-direction:column}.page-buttons{width:100%}.page-buttons button{flex:1}.dialog-actions{display:grid;grid-template-columns:1fr 1fr}.revision-seal{padding:12px}.toast{right:14px;bottom:14px}}
...
<main class="page">
  ...
  <section class="ledger" aria-labelledby="ledger-title">
    ...
    <div class="status-tabs" id="status-tabs" aria-label="按状态筛选">
      ...
      <button class="status-tab" type="button" data-status="awaiting_delivery" aria-pressed="false">等待评论</button>
    ...
    <form class="filters" id="filters">
      <div class="field"><label for="provider">代码平台</label><select id="provider" name="provider"><option value="">全部平台</option></select></div>
...
<dialog class="rerun-dialog" id="rerun-dialog" aria-labelledby="rerun-title">
  <div class="dialog-head"><div><h2 id="rerun-title">重新审查</h2><p id="rerun-subtitle">正在确认任务和版本</p></div><button class="dialog-close" id="rerun-close" type="button" aria-label="关闭">×</button></div>
  <div class="dialog-body" id="rerun-body"><div class="state-panel"><div><div class="state-symbol">…</div><strong>正在检查是否可以重新审查</strong></div></div></div>
  <div class="dialog-actions"><button class="quiet" id="rerun-cancel" type="button">取消</button><button class="primary" id="rerun-confirm" type="button" disabled>创建审查任务</button></div>
</dialog>
<div class="toast" id="rerun-toast" role="status" hidden><div><strong id="toast-title"></strong><span id="toast-detail"></span></div><button id="toast-view" type="button">查看任务</button></div>

<script>
const API='/api/v1/observability/review-runs';
const RUN_API='/api/v1/review-runs';
const PROVIDERS_API='/api/v1/providers';
const PAGE_SIZE=25;
const REFRESH_SECONDS=30;
const PROXY_TOKEN=new URLSearchParams(window.location.search).get('token')||'';
const statusNames={awaiting_delivery:'等待核心评论',queued:'排队中',running:'运行中',completed:'已完成',failed:'失败',cancelled:'已取消',superseded:'已替代'};
const stageNames={placeholder_delivery_pending:'创建占位评论',placeholder_ready:'占位评论已就绪',start:'准备执行',starting:'启动 pi-agent',retrying_agent_start:'重试执行器',waiting_for_agent:'等待执行器',waiting_for_human:'等待人工输入',agent_completed:'收集审查结果',collecting_result:'收集审查结果',completed_delivery_pending:'回写审查结果',failed_delivery_pending:'回写失败状态',publishing_summary:'发布审查摘要',cleanup:'清理工作区',completed:'执行完成'};
...
function statusRail(run){const status=run.status;let first='done',middle='',last='',lastLabel='结束',line1='done',line2='';if(status==='awaiting_delivery'){first='active';line1='';lastLabel='结束'}else if(status==='queued'){first='active';line1='';lastLabel='结束'}else if(status==='running'){middle='active';line2='active'}else if(status==='completed'){middle='done';last='done';line2='done'}else if(status==='failed'){middle='done';last='failed';lastLabel='失败';line2='done'}else if(status==='cancelled'){middle=run.started_at?'done':'';last='cancelled';lastLabel='取消';line2=run.started_at?'done':''}else if(status==='superseded'){middle=run.started_at?'done':'';last='cancelled';lastLabel='替代';line2=run.started_at?'done':''}return `<div class="rail" aria-label="${esc(statusNames[status]||status)}"><span class="rail-step ${first}"><span class="rail-node"></span>入队</span><span class="rail-line ${line1}"></span><span class="rail-step ${middle}"><span class="rail-node"></span>执行</span><span class="rail-line ${line2}"></span><span class="rail-step ${last}"><span class="rail-node"></span>${lastLabel}</span></div>`}
function mrCell(run){const context=run.pull_request_context||{};const provider=String(run.provider||'').toLowerCase();const meta=providerMeta[provider]||{};const kind=String(meta.kind||provider);const prefix=kind==='gitlab'?'!':'#';const requestName=kind==='gitlab'?'Merge request':'Pull request';const title=context.title||`${requestName} ${prefix}${run.pull_request_number}`;const url=safeUrl(context.html_url);const titleHtml=url?`<a class="mr-title" href="${esc(url)}" target="_blank" rel="noopener noreferrer" title="${esc(title)}">${esc(title)}</a>`:`<span class="mr-title" title="${esc(title)}">${esc(title)}</span>`;const mark=String(meta.display_name||provider||'?').split(/[\s_-]+/).map(part=>part[0]||'').join('').slice(0,3).toUpperCase();const origin=run.trigger_type==='dashboard_rerun'?' · 看板重审':run.trigger_type==='retry'?' · 失败重试':'';return `<div class="mr"><span class="provider-mark ${kind==='gitlab'?'gitlab':''}" aria-hidden="true">${esc(mark)}</span><span>${titleHtml}<span class="mr-number">${prefix}${esc(run.pull_request_number)} · 第 ${esc(run.attempt)} 次执行${origin}</span></span></div>`}
function reviewAction(run){if(run.status==='failed')return `<button class="retry-button" type="button" data-review-action="retry" data-run="${esc(run.id)}" aria-label="重试第 ${esc(run.pull_request_number)} 号任务">重试</button>`;if(['completed','cancelled'].includes(run.status))return `<button class="rerun-button" type="button" data-review-action="rerun" data-run="${esc(run.id)}" aria-label="重新审查第 ${esc(run.pull_request_number)} 号任务">重新审查</button>`;if(run.status==='superseded')return '<span class="action-muted">版本已过期</span>';return '<span class="action-muted">—</span>'}
function renderRows(items){return `<div class="table-wrap"><table><thead><tr><th>MR / PR</th><th>仓库</th><th>执行状态</th><th>版本</th><th>耗时</th><th>最近更新</th><th>操作</th></tr></thead><tbody>${items.map(run=>{const context=run.pull_request_context||{};const branch=context.head_ref&&context.base_ref?`${context.head_ref} → ${context.base_ref}`:(context.head_ref||context.base_ref||'');const stage=stageNames[run.stage]||run.stage||'等待分配阶段';const failure=run.status==='failed'&&(run.failure_code||run.error)?`<div class="failure" title="${esc(run.error||run.failure_code)}">${esc(run.failure_code||'执行失败')}${run.error?` · ${esc(run.error)}`:''}</div>`:'';return `<tr data-run-id="${esc(run.id)}" class="${state.highlightRunId===run.id?'highlight':''}"><td data-label="MR / PR">${mrCell(run)}</td><td data-label="仓库"><div class="repo" title="${esc(run.repo_full_name)}">${esc(run.repo_full_name)}</div>${branch?`<div class="branch" title="${esc(branch)}">${esc(branch)}</div>`:''}</td><td data-label="执行状态" class="run-cell">${statusRail(run)}<div class="stage">当前阶段 · <code>${esc(stage)}</code></div>${failure}</td><td data-label="版本"><code class="sha" title="${esc(run.head_sha)}">${esc(String(run.head_sha||'').slice(0,8)||'—')}</code></td><td data-label="耗时"><div class="time"><strong>${esc(duration(run))}</strong><span>${run.started_at?'已开始':'从创建起'}</span></div></td><td data-label="最近更新"><div class="time" title="${esc(exactTime(run.updated_at))}"><strong>${esc(relativeTime(run.updated_at))}</strong><span>${esc(exactTime(run.updated_at))}</span></div></td><td data-label="操作">${reviewAction(run)}</td></tr>`}).join('')}</tbody></table></div>`}
...
function requestHeaders(json=false){const headers={Accept:'application/json'};if(PROXY_TOKEN)headers['X-Review-Token']=PROXY_TOKEN;if(json)headers['Content-Type']='application/json';return headers}
...
function readUrl(){const query=new URLSearchParams(location.search);state.status=statusNames[query.get('status')]?query.get('status'):'';const provider=query.get('provider')||'';state.provider=/^[a-z0-9][a-z0-9_-]{0,63}$/.test(provider)?provider:'';state.repo_full_name=(query.get('repo_full_name')||'').slice(0,512);state.pull_request_number=/^[1-9]\d*$/.test(query.get('pull_request_number')||'')?query.get('pull_request_number'):'';state.page=Math.max(1,Number.parseInt(query.get('page')||'1',10)||1);syncControls()}
async function loadProviders(){try{const response=await fetch(PROVIDERS_API,{headers:requestHeaders()});if(!response.ok)return;const data=await response.json();const items=Array.isArray(data.items)?data.items:[];for(const item of items){providerMeta[item.key]=item;const option=document.createElement('option');option.value=item.key;option.textContent=item.display_name||item.key;$('#provider').appendChild(option)}syncControls()}catch{}}
...
function newRequestId(){const bytes=crypto.getRandomValues(new Uint8Array(16));bytes[6]=(bytes[6]&15)|64;bytes[8]=(bytes[8]&63)|128;const hex=[...bytes].map(value=>value.toString(16).padStart(2,'0')).join('');return `${hex.slice(0,8)}-${hex.slice(8,12)}-${hex.slice(12,16)}-${hex.slice(16,20)}-${hex.slice(20)}`}
async function responseError(response){if(response.status===401||response.status===403)return '当前访问凭据无权创建审查任务。';try{const payload=await response.json();const detail=payload.detail;if(detail&&typeof detail==='object'&&detail.message)return detail.message;if(typeof detail==='string')return detail}catch{}return `创建审查任务失败（${response.status}），请检查任务状态后重试。`}
function closeRerun(){if($('#rerun-dialog').open)$('#rerun-dialog').close();state.rerun=null}
function unavailableBody(action,kind){const current=action.current_head_sha?`<p class="dialog-note">当前 Head：<code>${esc(String(action.current_head_sha).slice(0,8))}</code></p>`:'';const verb=kind==='retry'?'重试':'重新审查';return `<div class="dialog-error"><strong>当前不能${verb}</strong><br>${esc(action.message||'任务状态已经发生变化。')}</div>${current}`}
async function openReviewAction(runId,kind,button){const verb=kind==='retry'?'重试':'重新审查';button.disabled=true;state.rerun=null;$('#rerun-title').textContent=kind==='retry'?'重试失败审查':'重新审查';$('#rerun-subtitle').textContent='正在确认任务和版本';$('#rerun-body').innerHTML=`<div class="state-panel"><div><div class="state-symbol">…</div><strong>正在检查是否可以${verb}</strong></div></div>`;$('#rerun-confirm').disabled=true;$('#rerun-confirm').textContent=kind==='retry'?'创建重试任务':'创建审查任务';if(!$('#rerun-dialog').open)$('#rerun-dialog').showModal();try{const response=await fetch(`${API}/${encodeURIComponent(runId)}`,{headers:requestHeaders()});if(!response.ok)throw new Error(errorMessage(response));const detail=await response.json();const action=detail.available_actions?.[kind]||{allowed:false,message:`服务没有返回${verb}能力。`};$('#rerun-subtitle').textContent=`${detail.repo_full_name} · ${detail.provider==='gitlab'?'!':'#'}${detail.pull_request_number}`;if(!action.allowed){$('#rerun-body').innerHTML=unavailableBody(action,kind);return}const base=String(detail.base_sha||'').slice(0,8)||'—';const head=String(detail.head_sha||'').slice(0,8)||'—';$('#rerun-body').innerHTML=`<div class="revision-seal"><span><small>Base commit</small><code>${esc(base)}</code></span><b>…</b><span><small>Head commit</small><code>${esc(head)}</code></span></div><p class="attempt-shift">第 ${esc(detail.attempt)} 次执行 → 将创建第 ${esc(action.next_attempt)} 次执行</p><p class="dialog-note">原审查结果和 Session 会保留。新任务将使用独立 Session 完整审查当前版本。</p><div class="dialog-error" id="rerun-error" hidden></div>`;state.rerun={runId,kind,requestId:newRequestId(),nextAttempt:action.next_attempt};$('#rerun-confirm').disabled=false}catch(error){$('#rerun-body').innerHTML=`<div class="dialog-error"><strong>无法检查任务</strong><br>${esc(error.message)}</div>`}finally{button.disabled=false}}
function bindReviewActions(){$$('#results [data-review-action]').forEach(button=>button.addEventListener('click',()=>openReviewAction(button.dataset.run,button.dataset.reviewAction,button)))}
async function submitRerun(){if(!state.rerun)return;const current=state.rerun;const isRetry=current.kind==='retry';const button=$('#rerun-confirm');button.disabled=true;button.textContent='正在创建…';const errorBox=$('#rerun-error');if(errorBox)errorBox.hidden=true;try{const options={method:'POST',headers:requestHeaders(!isRetry)};if(!isRetry)options.body=JSON.stringify({revision:'same_revision',idempotency_key:current.requestId});const response=await fetch(`${RUN_API}/${encodeURIComponent(current.runId)}/${current.kind}`,options);if(!response.ok)throw new Error(await responseError(response));const result=await response.json();state.createdRunId=result.review_run_id;state.highlightRunId=result.review_run_id;closeRerun();$('#toast-title').textContent=`第 ${result.attempt} 次审查正在创建占位评论`;$('#toast-detail').textContent=`任务 ${String(result.review_run_id).slice(0,8)} · 触发事件 ${String(result.review_request_event_id).slice(0,8)}`;$('#rerun-toast').hidden=false;$('#announcer').textContent=`第 ${result.attempt} 次审查已提交占位评论事件`;loadRuns({background:true,announce:false})}catch(error){if(errorBox){errorBox.innerHTML=`<strong>没有创建任务</strong><br>${esc(error.message)}`;errorBox.hidden=false}else{$('#rerun-body').insertAdjacentHTML('beforeend',`<div class="dialog-error"><strong>没有创建任务</strong><br>${esc(error.message)}</div>`)}}finally{button.disabled=false;button.textContent=isRetry?'创建重试任务':'创建审查任务'}}
async function loadRuns({background=false,announce=true}={}){if(state.loading&&background)return;state.controller?.abort();const controller=new AbortController();state.controller=controller;state.loading=true;$('#refresh').disabled=true;$('#refresh').classList.add('is-loading');$('#results').setAttribute('aria-busy','true');if(!background)$('#results').innerHTML=renderLoading();try{const response=await fetch(`${API}?${params()}`,{headers:requestHeaders(),signal:controller.signal});if(!response.ok)throw new Error(errorMessage(response));const data=await response.json();state.total=Number(data.total)||0;const items=Array.isArray(data.items)?data.items:[];$('#results').innerHTML=items.length?renderRows(items):renderEmpty();bindReviewActions();$('#result-summary').textContent=`共 ${state.total} 次执行，本页显示 ${items.length} 条`;if(updatePagination()){setTimeout(()=>loadRuns(),0);return}const now=new Date();$('#updated-at').textContent=`更新于 ${new Intl.DateTimeFormat('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(now)}`;if(announce)$('#announcer').textContent=`Review 列表已更新，共 ${state.total} 条记录`;state.remaining=REFRESH_SECONDS;syncUrl()}catch(error){if(error.name==='AbortError')return;$('#results').innerHTML=renderError(error.message||'无法连接 review 服务，请检查网络后重试。');$('#result-summary').textContent='未能取得任务数据';$('#pagination').hidden=true;$('#announcer').textContent='Review 列表加载失败'}finally{if(state.controller===controller){state.loading=false;$('#refresh').disabled=false;$('#refresh').classList.remove('is-loading');$('#results').setAttribute('aria-busy','false');updateRefreshLabel()}}}
...
$('#rerun-close').addEventListener('click',closeRerun);
$('#rerun-cancel').addEventListener('click',closeRerun);
$('#rerun-confirm').addEventListener('click',submitRerun);
$('#rerun-dialog').addEventListener('cancel',event=>{event.preventDefault();closeRerun()});
$('#toast-view').addEventListener('click',()=>{state.status='';state.page=1;syncControls();$('#rerun-toast').hidden=true;loadRuns().then(()=>{const row=document.querySelector(`[data-run-id="${CSS.escape(state.createdRunId||'')}"]`);row?.scrollIntoView({block:'center',behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})})});
...
readUrl();loadProviders().finally(()=>loadRuns());
</script>
</body>
</html>"""

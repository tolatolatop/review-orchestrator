...
function mrCell(run){const context=run.pull_request_context||{};const provider=String(run.provider||'').toLowerCase();const prefix=provider==='gitlab'?'!':'#';const title=context.title||`${provider==='gitlab'?'Merge request':'Pull request'} ${prefix}${run.pull_request_number}`;const url=safeUrl(context.html_url);const titleHtml=url?`<a class="mr-title" href="${esc(url)}" target="_blank" rel="noopener noreferrer" title="${esc(title)}">${esc(title)}</a>`:`<span class="mr-title" title="${esc(title)}">${esc(title)}</span>`;const origin=run.trigger_type==='dashboard_rerun'?' · 看板重审':run.trigger_type==='retry'?' · 失败重试':'';return `<div class="mr"><span class="provider-mark ${provider==='gitlab'?'gitlab':''}" aria-hidden="true">${provider==='gitlab'?'GL':'GH'}</span><span>${titleHtml}<span class="mr-number">${prefix}${esc(run.pull_request_number)} · 第 ${esc(run.attempt)} 次执行${origin}</span></span></div>`}

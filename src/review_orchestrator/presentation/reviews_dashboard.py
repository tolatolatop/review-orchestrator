...
async function submitRerun(){if(!state.rerun)return;const current=state.rerun;const isRetry=current.kind==='retry';const button=$('#rerun-confirm');button.disabled=true;button.textContent='正在创建…';const errorBox=$('#rerun-error');if(errorBox)errorBox.hidden=true;try{const options={method:'POST',headers:requestHeaders(!isRetry)};if(!isRetry)options.body=JSON.stringify({revision:'same_revision',idempotency_key:current.requestId});const response=await fetch(`${RUN_API}/${encodeURIComponent(current.runId)}/${current.kind}`,options);if(!response.ok)throw new Error(await responseError(response));const result=await response.json();state.createdRunId=result.review_run_id;state.highlightRunId=result.review_run_id;closeRerun();$('#toast-title').textContent=`第 ${result.attempt} 次审查已加入队列`;$('#toast-detail').textContent=`任务 ${String(result.review_run_id).slice(0,8)} · 触发事件 ${String(result.review_request_event_id).slice(0,8)}`;$('#rerun-toast').hidden=false;$('#announcer').textContent=`第 ${result.attempt} 次审查已加入队列`;loadRuns({background:true,announce:false})}catch(error){if(errorBox){errorBox.innerHTML=`<strong>没有创建任务</strong><br>${esc(error.message)}`;errorBox.hidden=false}else{$('#rerun-body').insertAdjacentHTML('beforeend',`<div class="dialog-error"><strong>没有创建任务</strong><br>${esc(error.message)}</div>`)}}finally{button.disabled=false;button.textContent=isRetry?'创建重试任务':'创建审查任务'}}
